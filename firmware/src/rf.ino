...
boolean RF_Rx_Handle()
{
        ...
        if (rf12_crc == 0) { //Check packet is good
                ...
                serial_print_frame(rf12_len); //Print recieved data
                if (RF12_WANTS_ACK==1) {
                        // Serial.print(F(" -> ack"));
                        rf12_sendStart(RF12_ACK_REPLY, 0, 0);
                }
                return 1;
        } else { // the packet is bad
                if (quiet_mode == 0) {
                        Serial.print(F(" ?")); //Print the "bad packet" line prefix
                        serial_print_frame(20); //Print only the first 20 bytes of a bad packet
                }
                return 0;
        }
}
...
void serial_print_frame (int len)
{
        Serial.print(F(" "));
